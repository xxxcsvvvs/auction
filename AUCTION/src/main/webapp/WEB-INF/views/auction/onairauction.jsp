...
<style>
    *{
    ...
    .mid{
        border-radius: 18px; height: 743px; 
    ...
    .chat-mid-cover{
    height: 485px; border-radius: 17px; overflow: hidden;
    }
    .chat-mid{
        height: 485px; box-sizing: border-box;
        color: green; border: 1px solid #ccc;
        line-height: 485px; margin-bottom: 2px;
    ...
    .swiper-body {
        position: relative;
        height: 450px; width: 100%; margin-top: 10px; border-radius: 20px; overflow: hidden;
    }
    .swiper {
        width: 100%;
        height: 100%;
	...
	#modal_open_btn4{width: 100%; height: 100%;
}
.body-container{
...
<c:if test="${user != null}">
        ...
        <h4 style="color: #28a745">Server Time ${nowTime}</h3>
            ...
            <div class="product-seller">
                <ul>
                    ...
                    <li class="pay">구매만족도
                        <strong>${auctionSeller.me_customer_certification}</strong>
                    </li>
                    <li class="recall">
                        반품수용여부
        ...
        <div class="modal_layer modal_layer4">
            <div id="modal">
                <div class="modal_content3">
                        ...
                        <input type="hidden" name= au_num value="${auction.getAu_num()}">
                </div>
            ...
            <div id="modal">
                <div class="modal_content3">
                    <c:forEach items="${boardList}" var="bl" varStatus="vs">
					      <label>
					        <input type="radio" id="delivery_num" name="bl_num" value="${bl.bl_num}" checked>
    ...
    $('.itemid .like_btn').click(function () {
    	...
    	//let pr_code = ${auctionProduct.getPr_code()} // let pr_code = '${auctionProduct.getPr_code()}''
    	//console.log(pr_code);
    	let formData = {
    			value1 : productLikeState,
    			value2 : product
    	}
    		$.ajax({
    			type: 'POST',
    ...
    <script type="text/javascript">
        ...
        function openSocket(){
            ...
            ws = new WebSocket("ws://localhost:8080/auction/echo.do/${chattingChannel}/${userId}");
